...
# 定义UFG模型
class UFGConv(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super(UFGConv, self).__init__()
        self.cheb1 = ChebConv(in_channels, out_channels, K=2)
        self.cheb2 = ChebConv(in_channels, out_channels, K=2)

    def forward(self, x, edge_index):
        x1 = self.cheb1(x, edge_index)
        x2 = self.cheb2(x, edge_index)
        return x1 + x2

class UFG(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels):
        super(UFG, self).__init__()
...
# 创建模型
model = UFG(num_node_features=1, hidden_channels=64).to(device)  # 增加隐藏层的神经元数量
