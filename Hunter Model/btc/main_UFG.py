...
class UFGConv(MessagePassing):
    def __init__(self, in_channels, out_channels, K=3):
        super(UFGConv, self).__init__(aggr='add')
        self.K = K
    ...
    def forward(self, x, edge_index):
        ...
        for k in range(1, self.K + 1):
            Tx_k = self.propagate(edge_index, x=Tx_0)
            out += torch.matmul(Tx_k, self.weight[k])
            Tx_0 = Tx_k
...
class UFGNet(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels):
        ...
        self.conv1 = UFGConv(num_node_features, hidden_channels)
        self.conv2 = UFGConv(hidden_channels, hidden_channels)
        self.fc = nn.Linear(hidden_channels, 1)  # 二分类全连接层
    ...
    def forward(self, data):
        ...
        x = self.leaky_relu(x)  # LeakyReLU激活
        ...
        x = self.fc(x)  # 全连接层
        return x.view(-1)  # 返回一维张量


# 创建模型
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = UFGNet(num_node_features=1, hidden_channels=64).to(device)
...
print(
    f"Predicted Positive Samples: {predicted_positive} ({predicted_positive / (predicted_positive + predicted_negative):.2%})")
print(
    f"Predicted Negative Samples: {predicted_negative} ({predicted_negative / (predicted_positive + predicted_negative):.2%})")
