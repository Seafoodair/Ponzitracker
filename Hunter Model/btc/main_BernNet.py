...
# 定义BernNet模型
class BernNet(MessagePassing):
    def __init__(self, in_channels, out_channels, K=10, bias=True):
        super(BernNet, self).__init__(aggr='add')
        self.K = K
    ...
    def reset_parameters(self):
        uniform(self.K, self.weight)
        uniform(self.K, self.bias)
    ...
    def forward(self, x, edge_index):
        ...
        for k in range(2, self.K + 1):
            Tx_2 = 2 * self.propagate(edge_index, x=Tx_1) - Tx_0
...
class BernNetModel(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, K=10):
        super(BernNetModel, self).__init__()
        self.conv1 = BernNet(num_node_features, hidden_channels, K=K)
        self.conv2 = BernNet(hidden_channels, hidden_channels, K=K)
...
model = BernNetModel(num_node_features=1, hidden_channels=64).to(device)  # 增加隐藏层的神经元数量

# 设置损失函数
loss_fn = torch.nn.BCEWithLogitsLoss().to(device)

# 使用学习率调度器
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)  # 调小学习率
